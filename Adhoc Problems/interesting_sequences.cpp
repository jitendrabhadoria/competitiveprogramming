...
long long findCost(int arr[], int n, int k, int l, int numToCheck, bool& possible){
    long long inc = 0;
    long long dec = 0;
    long long cost = 0;
    for(int i = 0 ; i<n ; i++){
        if(arr[i]<numToCheck){
            inc = inc + (numToCheck - arr[i]);
        } else if (arr[i]>numToCheck){
            dec = dec + (arr[i]-numToCheck);
        }
    }
    if(inc>=dec){
        possible = true;
...
int main(){
    ...
    for(long long i = min ; i <= max ; i++){
        bool possible = false;
        costForThis = findCost(arr,n,k,l,i,possible);
        if(possible){
            if(costForThis < minCost){
            	minCost = costForThis;
            }
        } else {
            continue;
