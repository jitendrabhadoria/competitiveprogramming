...
On substituting these values to the formula of totalSalary, we get Rs. 8761.53 and now rounding it off will result in 
...
int main() {
	long double base_sal;
    char grade;
    long double hra, da, allowance, pf, total;
    int fin_tot;
    cin>>base_sal>>grade;
    // if(base_sal<=7500000 && base_sal>=0){
    hra=0.20*base_sal;
    da=0.50*base_sal;
    pf=0.11*base_sal;
    if(grade=='A'){
    ...
    total=(base_sal+hra+da+allowance)-pf;
    fin_tot=round(total);
    cout<<fin_tot;
    // }
