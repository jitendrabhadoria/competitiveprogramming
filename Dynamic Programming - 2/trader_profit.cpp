...
int maximum_profit(int *arr, int n, int k, int ongoing_transaction, int ***dp){
    
    //the value of ongoing_transaction could be either 0 or 1. I am using it as a boolean.
	...
	int option1 = maximum_profit(arr+1, n-1, k, ongoing_transaction, dp); //ignore
    
	int option2;
    
	if(ongoing_transaction==1){
        
		option2 = maximum_profit(arr+1, n-1, k-1, 0, dp) + arr[0]; // sell
        
	} else {
        
		if(k>0){
            
			option2 = maximum_profit(arr+1, n-1, k, 1, dp) - arr[0]; // purchase
