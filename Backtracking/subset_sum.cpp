...
void findSubset(long long arr[], long long n, long long k, long long sumSoFar, long long sumRemaining){
   if(sumSoFar==k){
      globalCount++;
      return;
   }
   ...
   long long currentNumber = *arr;
   long long newSumRemaining = sumRemaining-currentNumber;

   //if number not taken
   findSubset(arr+1, n-1, k, sumSoFar, newSumRemaining);

   //if number taken
   long long newSumSoFar = sumSoFar + currentNumber;
   findSubset(arr+1, n-1, k, newSumSoFar, newSumRemaining);
...
int main() {
   ...
   while(t--){
      ...
      if(k==0){
         ...
         cout<<pow(2,globalCount)<<endl;
      }

      else {
         globalCount = 0;
         findSubset(arr, n, k, 0, sumRem);
