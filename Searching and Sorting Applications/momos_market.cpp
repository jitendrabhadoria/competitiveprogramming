...
int main(){
    ...
    while(q--){
        long long x;
        long long numMomo, savings;
        cin>>x;
        long long position = upper_bound(sumArr, sumArr+n, x) - sumArr;
        long long tempPos = position - 1;
        if(sumArr[tempPos]==x){
            numMomo = tempPos+1;
            savings = 0;
        } else if(sumArr[tempPos]<x){
            numMomo = tempPos+1;
            savings = x-sumArr[tempPos];
        }
        cout<<numMomo<<" "<<savings<<endl;
    }
